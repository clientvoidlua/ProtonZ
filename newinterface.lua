...
if rawget(_G, "UI_LOADED") then
    if Detectedly and Detectedly.toast then
        clonefunction(Detectedly.toast)("Proton Z is already running")
    end
    return
end
_G.UI_LOADED = true
...
do
    local ok, err = pcall(function()
        Interface.writefile = clonefunction(Detectedly.writefile)
        Interface.runcode = clonefunction(Detectedly.runcode)
        Interface.toast = clonefunction(Detectedly.toast)
        Interface.pushautoexec = clonefunction(Detectedly.pushautoexec)
    end)

    if not ok then
        warn("Failed to clone Detectedly functions:", err)
        return
    end
end
...
local function fetchJsonText()
    ...
    if not ok or type(data) ~= "table" then
        return "Failed to fetch changelogs."
    end

    local output = {}
    output[#output+1] = "Date: " .. tostring(data.date or "Unknown")
    output[#output+1] = "Changelogs:"

    if type(data.changelogs) == "table" then
        for i = 1, #data.changelogs do
            output[#output+1] = "- " .. tostring(data.changelogs[i])
        end
    end
...
pcall(function()
    if Interface.pushautoexec then
